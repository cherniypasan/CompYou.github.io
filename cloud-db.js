...
class CloudDB {
    constructor() {
        ...
        this.API_URL = 'https://script.google.com/macros/s/AKfycbwXpkXccTSxTSiClwOQpDe6al7WJbMr9rYdSq91OAAZElORVT5SKWJVs_yDQsxLN762SQ/exec'; // ВАЖНО: Замените на ваш URL после настройки
    ...
    async loadAllOrders() {
        ...
        try {
            ...
            console.warn('Cloud DB: Не удалось загрузить из облака, используем локальные:', error.message);
            return localOrders;
        }
    }
    ...
    async syncOrders() {
        ...
        if (!this.useCloud) {
            console.log('Cloud DB: Облако отключено, синхронизация невозможна');
        ...
        try {
            ...
            if (result.success && Array.isArray(result.orders)) {
                ...
                // Находим заказы, которые есть локально, но нет в облаке
                const ordersToUpload = localOrders.filter(localOrder => 
                    !cloudOrders.some(cloudOrder => cloudOrder.id === localOrder.id)
                );
                
                // Загружаем отсутствующие заказы в облако
                let uploadedCount = 0;
                let uploadErrors = [];
                
                for (const order of ordersToUpload) {
                    try {
                        const uploadResult = await this.saveOrder(order);
                        if (uploadResult.success && uploadResult.cloudSaved) {
                            uploadedCount++;
                        } else if (uploadResult.cloudError) {
                            uploadErrors.push(`Заказ #${order.id}: ${uploadResult.cloudError}`);
                        }
                    } catch (error) {
                        uploadErrors.push(`Заказ #${order.id}: ${error.message}`);
                    }
                }
                ...
                console.log(`Cloud DB: Синхронизация завершена. Загружено в облако: ${uploadedCount}, ошибок: ${uploadErrors.length}`);
                
                return {
                    success: true,
                    stats: {
                        totalLocal: localOrders.length,
                        totalCloud: cloudOrders.length,
